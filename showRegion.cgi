...
sub getSequenceData {
    ...
    $seq =~ s/(.{80})/$1\<br\>/g;
    $seq = '>id_'.$id.' length:'.$length.$q->br().$seq;

    $stm = 'SELECT source.name,accession,linkout,contig,strand,start16S,stop16S,start23S,stop23S FROM region LEFT JOIN (region_entry,entry,source) ON (region.id=region_entry.region_id AND region_entry.entry_id=entry.id AND entry.source_id=source.id) WHERE region.id='.$id;
...
#    die Dumper($aref);
    ...
    foreach(@$aref) {
	my (@content,$linkout);
	my ($db,$acc,$link,$contig,$strand,$start16S,$stop16S,$start23S,$stop23S) = @$_;
	...
	push(@content,['16S start',$start16S]);
	push(@content,['16S stop',$stop16S]);
	push(@content,['23S start',$start23S]);
	push(@content,['23S stop',$stop23S]);
